...
for mp in meta_files:
    ...
    if mp.stem != canonical_stem(data): errors.append(f"noncanonical title-derived filename: {vid}")
    ...
    if raw.name != mp.stem: errors.append(f"metadata/raw basename mismatch: {vid}")
...
if len(csv_ids) != len(ids) or set(csv_ids) != ids: errors.append("metadata/videos.csv does not exactly match metadata files")
